	...
	function KCAPTCHA(){
		...
		do{
            ...
            $backColor = imagecolorallocate($image,
                (int)($backColorT% 0x1000000 / 0x10000),
                (int)($backColorT % 0x10000 / 0x100),
                $backColorT % 0x100);
            imagefilledrectangle($image,0,0,$width,$height,$backColor);
            imagecolordeallocate($image,$backColor);

            $foreColor = imagecolorallocate($image,
                (int)($foreColorT % 0x1000000 / 0x10000),
                (int)($foreColorT% 0x10000 / 0x100),
                $foreColorT % 0x100);

            $fontFile = dirname(__FILE__) . '/fonts/Duality.ttf';

            $length = strlen($this->keystring);
            $box = imagettfbbox(30,0,$fontFile,$this->keystring);
            $w = $box[4] - $box[0] -2 * ($length - 1);
            $h = $box[1] - $box[5];
            $scale = min(($width - 2* 2) / $w,($height - 2 * 2) / $h);
            $x = 10;
